...
import { createQuiz, createQuestion } from '@/actions/quizzes';
import Input from '@/components/ui/Input';
import Button from '@/components/ui/Button';

export default function QuizForm({ onSuccess, lessons = [], modules = [] }) {
    ...
    <div className="flex flex-col gap-6">
      {!quizCreated ? (
        <form onSubmit={handleCreateQuiz} className="flex flex-col gap-4">
          ...
          <Input name="title" label="Quiz Title" required />
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1 block">Quiz Type</label>
            <select name="quiz_type" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-navy-500">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input name="passing_score" label="Passing Score (%)" type="number" defaultValue="60" min="1" max="100" />
            <Input name="max_attempts" label="Max Attempts" type="number" defaultValue="3" min="1" max="10" />
          </div>
          <Input name="time_limit_minutes" label="Time Limit (minutes, optional)" type="number" min="1" />
          <Button type="submit" loading={loading} className="w-full">Create Quiz</Button>
        </form>
      ) : (
        <div>
